...
(function () {
  ...
  function smBuildDataIntegrityReportHTML(r, metadata, iconUri) { // eslint-disable-line no-unused-vars
    ...
    return '<!DOCTYPE html><html><head><meta charset="utf-8">' +
      ...
      '<div class="overview">' +
        '<strong>How to read this report.</strong> Every statistic the document reports was recomputed in code from the numbers the document itself prints — nothing here is estimated or judged by a model. A “Doesn’t match reported” result is arithmetic, stated with its working; what it means is a conversation, and each one carries a question to ask. ' +
        'Across the ' + stats.length + ' reported statistics: ' + countsInline +
      '</div>' +

      section('Recomputed Statistics', stats.length ? stats.map(statCard).join('') : '') +
